...
apply_permissions() {
    ...
    then
        ...
        ynh_exec_as_app sed -i "/permissions:/,/relay:/{/: $role/d;}" "$install_dir/config.yaml"
        # Ensure that entries with value surrounded with quotes are deleted too. E.g. "users".
        ynh_exec_as_app sed -i "/permissions:/,/relay:/{/: \"$role\"/d;}" "$install_dir/config.yaml"
      	for user in "${usersArray[@]}"
            do
              if grep -q -x "${user}" <<< "$allDefinedEntries"; then
                ynh_print_info "User $user already defined in another role."
              else
                ynh_exec_as_app sed -i "/permissions:/a \        \\\"$user\": $role" "$install_dir/config.yaml" # Whitespaces are needed so that the file can be correctly parsed
              fi
        done
    fi
    set +o noglob
